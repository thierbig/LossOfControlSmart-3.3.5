...
function LossOfControlFrame_OnLoad(self)
    ...
    -- Apply saved frame settings (size/position/drag) if options module is present
    if ( LossOfControlSmart and LossOfControlSmart.ApplyFrameSettings ) then
...
function LossOfControlFrame_OnEvent(event, self, ...)
    ...
    elseif ( event == "LOSS_OF_CONTROL_ADDED" and ... ) then
        local eventIndex = ...;
        local data = C_LossOfControlSmart.GetActiveLossOfControlData(eventIndex);
    ...
    elseif ( event == "VARIABLES_LOADED" ) then
        if ( LoC_CVar.GetCVarBool("lossOfControlSmart") ) then
            self:RegisterEvents("LOSS_OF_CONTROL_UPDATE");
            self:RegisterEvents("LOSS_OF_CONTROL_ADDED");
        end
    end
...
function LossOfControlFrame_SetUpDisplay(self, animate, data)
    if ( not data ) then
        data = C_LossOfControlSmart.GetActiveLossOfControlData(LOSS_OF_CONTROL_ACTIVE_INDEX);
    end
    ...
    if ( text and displayType ~= DISPLAY_TYPE_NONE ) then
        ...
        if ( animate ) then
            ...
            self.Cooldown:Hide();
            self.Anim:Play();
            -- Respect user preference for sound playback if options module is present
            if ( LossOfControlSmart and LossOfControlSmart.IsSoundEnabled ) then
                if ( LossOfControlSmart:IsSoundEnabled() ) then
...
function LossOfControlFrame_UpdateDisplay(self)
    ...
    local data = C_LossOfControlSmart.GetActiveLossOfControlData(LOSS_OF_CONTROL_ACTIVE_INDEX);
